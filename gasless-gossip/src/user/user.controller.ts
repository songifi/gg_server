...
import { 
  ...
  export class UserController {
    ...
    async findAll(
      ...
      if (page < 1) {
        throw new BadRequestException('Page should be greater than or equal to 1');
      }
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Search users by username, display name or wallet address' })
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Username availability checked', type: Object })
    ...
    @ApiResponse({ status: HttpStatus.OK, description: 'Profile retrieved successfully', type: UserProfileDto })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'User not found' })
    async getProfile(
    ...
    @ApiParam({ name: 'id', description: 'User ID' })
    @ApiResponse({ status: HttpStatus.OK, description: 'Wallet address associated successfully', type: UserResponseDto })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'User not found' })
    @ApiResponse({ status: HttpStatus.CONFLICT, description: 'Wallet address already associated with another user' })
