...
export class ContactsService {
   ...
   */
  async createContact(ownerId: string, createContactDto: CreateContactDto): Promise<ContactResponseDto> {
  ...
  async isUserBlocked(ownerId: string, userId: string): Promise<boolean> {
    const blockedContact = await this.contactModel.findOne({
      owner: ownerId,
      user: userId,
      status: ContactStatus.BLOCKED,
    }).exec();
  ...
  async createContactGroup(
    ...
    const existingGroup = await this.contactGroupModel.findOne({
      owner: ownerId,
  ...
  async getContactGroups(ownerId: string): Promise<ContactGroupResponseDto[]> {
    const groups = await this.contactGroupModel.find({
      owner: ownerId,
    }).sort({ name: 1 }).exec();
